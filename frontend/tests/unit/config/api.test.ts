...
describe('api config', () => {
  ...
  it('should return window hostname with port 4444 for API and socket in PROD', () => {
    ...
    expect(getApiBaseUrl()).toBe(`${window.location.protocol}//${window.location.hostname}:3333`);
    expect(getSocketUrl()).toBe(`${window.location.protocol}//${window.location.hostname}:3333`);
  });
});
